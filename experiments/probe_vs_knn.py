"""Retrieval against a trained probe, under one shared certificate.

Usage: python experiments/probe_vs_knn.py [benchmark ...] [--mlp]

Loaders are the shared gate drivers' own, so the knn column here reproduces the
GUARD column of Table 3 and acts as this script's internal control.
"""
...
KINDS = ('knn', 'linear', 'mlp') if '--mlp' in sys.argv else ('knn', 'linear')
WANT = set(args) if args else {'AVE', 'NinaPro', 'PTB-XL', 'IEMOCAP', 'DrugBAN'}
SUF = os.environ.get('PROBE_SUFFIX', '')
OUT = str(_ROOT / f'results/gates/probe_vs_knn{SUF}.json')
res = {}
...
def report(name, acc):
    ...
    for k in KINDS:
        r = {f: float(np.nanmean([x[f] for x in acc[k]]))
             for f in ('gain', 'harm', 'blanket_gain', 'blanket_harm', 'apply', 'base')}
        out[k] = r
        print(f"  {k:8}gain {r['gain']:+.4f}  harm {r['harm']:.3f}  "
              f"apply {r['apply']:.3f}  (blanket gain {r['blanket_gain']:+.4f})", flush=True)
...
if 'PTB-XL' in WANT:
    ...
    for c, fk in (('a', 'a'), ('t', 't'), ('v', 'v'), ('at', 'a'), ('av', 'a'), ('tv', 't')):
        ...
        for seed in range(3):
            ...
            collect(acc, rows(P, F, Y, (np.arange(n), perm[:k], perm[k:2 * k], perm[2 * k:]),
                              loss_name='bernoulli', correctors=KINDS))
        ...
        for m in ['a', 't', 'v', 'at', 'av', 'tv', 'atv']:
            ...
            for cut in range(5):
                ...
                collect(acc, rows(P, F, Y, (np.arange(npool), parts[0], parts[1], parts[2]),
                                  targets=('hard', 'cross'), richer=Rh, correctors=KINDS))
